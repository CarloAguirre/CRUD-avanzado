...
const putUsuario = async(req, res = response)=> { 
    ...
    const usuario = await Usuario.findByIdAndUpdate(id, resto, {new: true}) // <--- el primer parametro es el id, y el segundo son los argumentos que vamos actualizar, y el tercero hace que retorne los valores actualizados
...
const deleteUsuario =  async(req, res = response)=> { 
    ...
    const usuarioBorrado = await Usuario.findByIdAndUpdate(id, {estado: false}, {new: true} ) //(El tercercer argumento sirve para enviar un usuario actualizado )
